...
namespace ns3 {
/**
 * OpenFlow control channel socket type
 */
enum SocketType {
	TCP = 0,         //!< Uses tcp socket
	UDP = 1,         //!< Uses udp socket
	PACKET = 2,      //!< Uses packet socket
	IPV4RAW = 3      //!< Uses ipv4 raw socket
};

class OFSwitch13Port;

struct UdpConnInfo {
	bool RecHello; // whether received a hello msg on this connection
	bool TxHello;  // whether sent a hello msg on this connection
	bool RecFeaRes; // whether received a featureReply to the last featureReq sent
	Time CreateTime; // the time when this connection is created
	Time LastFeatureReqTime; // the time when last featureReq is sent
	Time LastRecTime; // the time when the last msg is received
};

/*
 * the compare class for Address
 */
struct AddressComp {
...
public:
	...
	std::map<uint8_t, std::map<ofp_type, int> > m_conn_to_TXthr; //record the throughput of different packets sent to the controller in different connection
	std::map<uint8_t, std::map<ofp_type, int> > m_conn_to_RXthr; //record the throughput of different packets recieved from the controller in different connection
	std::map<uint32_t, double> m_pkt_in; // record the generating time of packet-in
	std::map<uint32_t, double> m_flow_setup; // the flow setup delay for each packet-in
	//static uint64_t m_globalPacketIn; // for debug, global counter for packet in
	/**
	 * Register this type.
	...
	/**
	 * \return the switch address of the main connection
	 */
	Address GetMainSwAddress(void) const;
	...
	/*
	 * set the auxiliary connections
	 * \param des the controller side ip address of the auxiliary connection
	 * \param src the switch side ip address of the auxiliary connection
	 * \param conn_id the id of the auxiliary connection
	 */
	void SetAuxConn(Address des, Address src, uint8_t conn_id);
	...
	/*
	 * starts the auxiliary connections between this switch and the controller
	 * \param v_conn_type, the connection type of the auxiliary connections
	 */
	void StartControllerAuxConnection(std::vector<SocketType>& v_conn_type);
	...
	DpActionsOutputPort(struct packet *pkt, uint32_t outPort, uint32_t outQueue,
			uint16_t maxLen, uint64_t cookie);
...
private:
	...
	void AuxiliaryConnTimeout(Address addr);

	void UdpAuxConnNoRecTimeout(Address addr);
	...
	/**
	 * Send a packet to the controller node with auxiliary connections.
	 * \see SendOpenflowBufferToRemote ().
	 * \param packet The ns-3 packet to send.
	 * \param conn_id the connection id of the connection which is used to transmit msg
	 * \return 0 if everything's ok, otherwise an error number.
	 */
	int SendToControllerWithAux(Ptr<Packet> packet, uint8_t conn_id);
	...
	/**
	 * Socket callback to receive a openflow udp packet from controller.
	 * \see remote_rconn_run () at udatapath/datapath.c.
	 * \param socket The udp socket.
	 */
	void ReceiveUdpFromController(Ptr<Socket> socket);
	...
	/**
	 * socket callback fired when a aux connection to controller succeed
	 */
	void SocketAuxCtrlSucceeded(Ptr<Socket> socket);
	...
	Ptr<Socket> m_ctrlSocket;   //!< Tcp main Socket to controller.
	Address m_swAddr;       //!< switch main address
	Address m_ctrlAddr;     //!< Controller main Address
	std::map<Address, Ptr<Socket>, AddressComp> m_auxConn; //map the destination address to connection socket
	std::map<Address, UdpConnInfo> m_udpConnStat; // map the destination address to the connection info
	std::map<Address, EventId> m_udpEventId; // map the destination address to the event id of checking whether no msg is received after last reception
	std::map<Address, Address> m_des_to_src; // map the destination address to source address
	std::map<uint8_t, Address> m_aux_id_to_addr; // map the conn id to the address
	std::map<Ptr<Socket>, PendingRecv> m_pendingSocket;

	Time m_timeout;      //!< Datapath timeout interval.
	Time m_noMsgTimeOut; // if no msg received on a unrealiable auxiliary connection after the connection is built, then terminate the connection
	Time m_noRecTimeOut; // if no msg received on a unrealiable auxiliary connection after last receive, then terminate the connection
	...
	bool m_isMainSetup;             // whether the main connection is setup
