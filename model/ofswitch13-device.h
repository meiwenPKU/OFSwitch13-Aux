...
public:
   ...
   * \param aux the controller side ip address of the auxiliary connection
   * \param conn_id the id of the auxiliary connection
   */
  void SetAuxConn(Address aux, uint8_t conn_id);
...
private:
  ...
  std::map<Address,Ptr<Socket>, AddressComp> m_auxConn; //map the address to connection socket
  std::map<uint8_t,Address> m_aux_id_to_addr; // map the conn id to the address
  //std::map<uint8_t, std::map<ofp_type,uint16_t> > m_conn_to_TXthr; //record the throughput of different packets sent to the controller in different connection
  //std::map<uint8_t, std::map<ofp_type,uint16_t> > m_conn_to_RXthr; //record the throughput of different packets recieved from the controller in different connection
