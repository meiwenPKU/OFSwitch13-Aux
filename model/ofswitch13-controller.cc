...
void
OFSwitch13Controller::RegisterConnection(uint8_t auxID, uint64_t datapathID, Ipv4Address source)
{
	m_connectionMap.insert(std::pair<Ipv4Address,std::pair<uint64_t, uint8_t> > (source,std::pair<uint8_t, uint64_t>(datapathID,auxID)));
}
                                    ...
                                    uint32_t xid)
{
  ...
  NS_LOG_DEBUG ("At " << Simulator::Now().GetSeconds() << "s, TX to switch " << swtch->ipv4 << ": " << msg_str);
  free (msg_str);
...
SwitchInfo OFSwitch13Controller::SelectRandomSW(SwitchInfo sw){
	int count = 0;
	for (std::map<Ipv4Address, SwitchInfo>::iterator it = m_switchesMap.begin(); it != m_switchesMap.end(); it++){
                                         ...
                                         uint32_t xid)
{
  ...
  //NS_LOG_DEBUG(Simulator::Now().GetSeconds() << ", controller receive a " << msg->type << "packet from switch " << swtch.ipv4);
...
OFSwitch13Controller::SocketAccept (Ptr<Socket> socket, const Address& from)
{
  ...
  //debug
  NS_LOG_DEBUG ("Switch request connection accepted from " << ipv4 << ", port = " << InetSocketAddress::ConvertFrom (from).GetPort());
  ...
  NS_LOG_DEBUG ("Switch main connection address  " << InetSocketAddress::ConvertFrom (swInfo->swDev->GetMainSwAddress()).GetIpv4 ()
		  << ", port = " << InetSocketAddress::ConvertFrom (swInfo->swDev->GetMainSwAddress()).GetPort ());

  //if (ipv4 != InetSocketAddress::ConvertFrom (swInfo->swDev->GetMainSwAddress()).GetIpv4 ())
	  //return;
  // Handshake messages
