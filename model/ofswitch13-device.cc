...
/********** Public methods **********/
TypeId OFSwitch13Device::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::OFSwitch13Device").SetParent<Object>().SetGroupName(
					"OFSwitch13").AddConstructor<OFSwitch13Device>().AddAttribute(
					"DatapathId",
					...
					MakeUintegerChecker<uint64_t>()).AddAttribute("PortList",
					"The list of ports associated to this switch.",
					ObjectVectorValue(),
					MakeObjectVectorAccessor(&OFSwitch13Device::m_ports),
					MakeObjectVectorChecker<OFSwitch13Port>()).AddAttribute(
					"TCAMDelay",
					...
					MakeTimeChecker()).AddAttribute("DatapathTimeout",
					"The interval between timeout operations on pipeline.",
					TimeValue(MilliSeconds(100)),
					MakeTimeAccessor(&OFSwitch13Device::m_timeout),
					MakeTimeChecker()).AddAttribute("NoMsgAfterStartTimeout",
					"The maximal interval for receiving no msg after the connection is started.",
					TimeValue(Seconds(5)),
					MakeTimeAccessor(&OFSwitch13Device::m_noMsgTimeOut),
					...
					MakeTimeAccessor(&OFSwitch13Device::m_noRecTimeOut),
					MakeTimeChecker()).AddAttribute("ControllerAddr",
					"The controller main InetSocketAddress.",
					AddressValue(
							InetSocketAddress(Ipv4Address("10.100.150.1"),
									6653)),
					MakeAddressAccessor(&OFSwitch13Device::m_ctrlAddr),
					MakeAddressChecker()).AddAttribute("SwitchAddr",
					"The switch main InetSocketAddress.",
					AddressValue(
							InetSocketAddress(Ipv4Address("10.100.150.0"),
									9999)),
					MakeAddressAccessor(&OFSwitch13Device::m_swAddr),
					MakeAddressChecker()).AddAttribute("LibLogLevel",
...
Ptr<OFSwitch13Port> OFSwitch13Device::AddSwitchPort(Ptr<NetDevice> portDevice) {
...
void OFSwitch13Device::ReceiveFromSwitchPort(Ptr<Packet> packet,
		uint32_t portNo) {
...
uint32_t OFSwitch13Device::GetNSwitchPorts(void) const {
	return m_datapath->ports_num;
}

Address OFSwitch13Device::GetMainSwAddress(void) const {
	return m_swAddr;
}
uint64_t OFSwitch13Device::GetDatapathId(void) const {
	return m_dpId;
}

uint32_t OFSwitch13Device::GetNumberFlowEntries(void) const {
	NS_LOG_FUNCTION(this);
...
uint32_t OFSwitch13Device::GetNumberFlowEntries(size_t tid) const {
	NS_ASSERT_MSG(m_datapath, "No datapath defined yet.");
...
void OFSwitch13Device::SetLibLogLevel(std::string log) {
...
void OFSwitch13Device::SetAuxConn(Address des, Address src, uint8_t conn_id) {
	NS_LOG_FUNCTION(this << des);
	m_auxConn.insert(std::pair<Address, Ptr<Socket> >(des, Ptr<Socket>()));
	m_aux_id_to_addr.insert(std::pair<uint8_t, Address>(conn_id, des));
	m_des_to_src.insert(std::pair<Address, Address>(des, src));
	//m_auxAdd.insert(std::pair<Address,Address>(desti,source));
}

void OFSwitch13Device::AuxiliaryConnTimeout(Address addr) {
	if (m_udpConnStat[addr].LastRecTime >= m_udpConnStat[addr].CreateTime) {
		NS_LOG_INFO(
				"Receive msg after the connection is setup before NoMsgAfterStartTimeout is timeout");
	} else {
		m_auxConn[addr]->Close();
	}

}

void OFSwitch13Device::StartControllerAuxConnection(
		std::vector<SocketType>& v_conn_type) {
	...
	int i = 0;
	for (std::map<Address, Ptr<Socket> >::iterator it = m_auxConn.begin();
			it != m_auxConn.end(); ++it) {
		NS_LOG_DEBUG("start auxiliary connection " << i+1);
		if (!it->second) {
			int error = 0;
			...
			if (v_conn_type[i] == TCP) {
				...
				Address src = m_des_to_src[it->first];
				Ipv4Address ipv4 =
						InetSocketAddress::ConvertFrom(src).GetIpv4();
				uint16_t port = InetSocketAddress::ConvertFrom(src).GetPort();
				error = it->second->Bind(InetSocketAddress(ipv4, port));
				if (error) {
					NS_LOG_ERROR("Error binding socket " << error);
					return;
				}

				error = it->second->Connect(
						InetSocketAddress::ConvertFrom(it->first));
				if (error) {
				...
				it->second->SetConnectCallback(
						MakeCallback(&OFSwitch13Device::SocketAuxCtrlSucceeded,
								this),
						MakeCallback(&OFSwitch13Device::SocketCtrlFailed,
								this));
			} else if (v_conn_type[i] == UDP) {

				UdpConnInfo udpInfo;
				udpInfo.CreateTime = Simulator::Now();
				udpInfo.RecHello = false;
				m_udpConnStat.insert(
						std::pair<Address, UdpConnInfo>(it->first, udpInfo));

				Simulator::Schedule(m_noMsgTimeOut,
						&OFSwitch13Device::AuxiliaryConnTimeout, this,
						it->first);
				...
				Ipv4Address ipv4 =
						InetSocketAddress::ConvertFrom(src).GetIpv4();
				uint16_t port = InetSocketAddress::ConvertFrom(src).GetPort();
				it->second->Bind(InetSocketAddress(ipv4, port));
				it->second->SetRecvCallback(
						MakeCallback(
								&OFSwitch13Device::ReceiveUdpFromController,
								this));
				it->second->Listen();
				error = it->second->Connect(
						InetSocketAddress::ConvertFrom(it->first));
...
void OFSwitch13Device::StartControllerConnection() {
	NS_LOG_FUNCTION(this);
	...
	if (!m_ctrlSocket) {
		...
		error = m_ctrlSocket->Bind(InetSocketAddress::ConvertFrom(m_swAddr));
		if (error) {
			NS_LOG_ERROR("Error binding socket " << error);
			return;
		}

		error = m_ctrlSocket->Connect(
				InetSocketAddress::ConvertFrom(m_ctrlAddr));
		if (error) {
			NS_LOG_ERROR("Error connecting socket " << error);
			return;
		}
...
Ptr<OFSwitch13Queue> OFSwitch13Device::GetOutputQueue(uint32_t portNo) {
...
int OFSwitch13Device::SendOpenflowBufferToRemote(ofpbuf *buffer, remote *ctrl) {
...
void OFSwitch13Device::DpActionsOutputPort(struct packet *pkt, uint32_t outPort,
...
void OFSwitch13Device::MeterCreatedCallback(struct meter_entry *entry) {
	Ptr<OFSwitch13Device> dev = OFSwitch13Device::GetDevice(entry->dp->id);
	dev->NotifyMeterEntryCreated(entry);
}

void OFSwitch13Device::MeterDropCallback(struct packet *pkt) {
	Ptr<OFSwitch13Device> dev = OFSwitch13Device::GetDevice(pkt->dp->id);
	dev->NotifyPacketDropped(pkt);
}

void OFSwitch13Device::PacketCloneCallback(struct packet *pkt,
		struct packet *clone) {
	Ptr<OFSwitch13Device> dev = OFSwitch13Device::GetDevice(pkt->dp->id);
	dev->NotifyPacketCloned(pkt, clone);
}

void OFSwitch13Device::PacketDestroyCallback(struct packet *pkt) {
	Ptr<OFSwitch13Device> dev = OFSwitch13Device::GetDevice(pkt->dp->id);
	dev->NotifyPacketDestroyed(pkt);
}

void OFSwitch13Device::BufferSaveCallback(struct packet *pkt, time_t timeout) {
	Ptr<OFSwitch13Device> dev = OFSwitch13Device::GetDevice(pkt->dp->id);
	dev->BufferPacketSave(pkt->ns3_uid, timeout);
}

void OFSwitch13Device::BufferRetrieveCallback(struct packet *pkt) {
	Ptr<OFSwitch13Device> dev = OFSwitch13Device::GetDevice(pkt->dp->id);
	dev->BufferPacketRetrieve(pkt->ns3_uid);
}

/********** Private methods **********/
void OFSwitch13Device::DoDispose() {
	NS_LOG_FUNCTION(this);
	...
	for (std::map<Address, Ptr<Socket> >::iterator it = m_auxConn.begin();
			it != m_auxConn.end(); it++) {
		Ptr<Socket> soc = it->second;
		soc->Dispose();
		it->second = NULL;
	}
	m_auxConn.clear();
	m_des_to_src.clear();
	m_aux_id_to_addr.clear();
	for (std::map<Ptr<Socket>, PendingRecv>::iterator it =
			m_pendingSocket.begin(); it != m_pendingSocket.end(); it++) {
		Ptr<Socket> soc = it->first;
		soc->Dispose();
	}
...
void OFSwitch13Device::DatapathTimeout(datapath* dp) {
	NS_LOG_FUNCTION(this);
	...
	m_lastTimeout = Simulator::Now();
	/*
	 if (m_lastTimeout.GetSeconds() > 22){
	 std::cout<< "debug here at : " << m_lastTimeout.GetSeconds() << std::endl;
	 }*/
	Simulator::Schedule(m_timeout, &OFSwitch13Device::DatapathTimeout, this,
			dp);
}

Ptr<OFSwitch13Port> OFSwitch13Device::GetOFSwitch13Port(uint32_t no) {
...
bool OFSwitch13Device::SendToSwitchPort(struct packet *pkt, uint32_t portNo,
		uint32_t queueNo) {
...
void OFSwitch13Device::SendToPipeline(Ptr<Packet> packet, uint32_t portNo) {
	NS_LOG_FUNCTION(this << packet);
...
int OFSwitch13Device::SendToControllerWithAux(Ptr<Packet> packet,
		uint8_t conn_id) {
	if (m_auxConn.size() == 0) {
	...
	} else // auxiliary connection
	{
		...
		if (m_auxConn[m_aux_id_to_addr[conn_id]]->GetTxAvailable()
				< packet->GetSize()) {
			NS_LOG_ERROR("Unavailable space to send OpenFlow message now.");
			Simulator::Schedule(m_timeout,
					&OFSwitch13Device::SendToControllerWithAux, this, packet,
					conn_id);
			return -1;
		}
		...
		NS_LOG_DEBUG(
				Simulator::Now().GetSeconds() << ", switch " << this->GetDatapathId() << " send a " << msg_str << " packet to controller through connection " << int(conn_id));
		...
		if (msg->type == OFPT_PACKET_IN) {
			ofl_msg_packet_in* msg_pkt_in = (ofl_msg_packet_in*) msg;
			if (m_pkt_in.find(msg_pkt_in->buffer_id) != m_pkt_in.end()) {
				NS_LOG_DEBUG("Duplicate Packet-In msg with same buffer id ");
			} else {
				m_pkt_in[msg_pkt_in->buffer_id] = Simulator::Now().GetSeconds();
			}
		}
		if ((m_conn_to_TXthr[conn_id]).find(msg->type)
				!= (m_conn_to_TXthr[conn_id]).end()) {
...
int OFSwitch13Device::SendToController(Ptr<Packet> packet) {
	...
	ofl_msg_unpack((uint8_t*) test_buffer->data, test_buffer->size, &test_msg,
			&test_xid, NULL);
	char *msg_str = ofl_msg_to_string(test_msg, NULL);
	NS_LOG_DEBUG(
			Simulator::Now().GetSeconds() << ", switch " << this->GetDatapathId() << " send a " << msg_str << " packet to controller through main connection");
	...
	if (test_msg->type == OFPT_PACKET_IN) {
		ofl_msg_packet_in* msg = (ofl_msg_packet_in*) test_msg;
		if (m_pkt_in.find(msg->buffer_id) != m_pkt_in.end()) {
			//std::cout << "duplicate pkt_in msg with same buffer id" << std::endl;
			NS_LOG_DEBUG("Duplicate Packet-In msg with same buffer id ");
		} else {
			m_pkt_in[msg->buffer_id] = Simulator::Now().GetSeconds();
		}
	}

	if ((m_conn_to_TXthr[0]).find(test_msg->type)
			!= (m_conn_to_TXthr[0]).end()) {
...
void OFSwitch13Device::UdpAuxConnNoRecTimeout(Address addr) {
...
void OFSwitch13Device::ReceiveUdpFromController(Ptr<Socket> socket) {
	NS_LOG_FUNCTION(this << socket);

	if (m_pendingSocket.find(socket) == m_pendingSocket.end()) {
		PendingRecv pendingSocket;
		pendingSocket.pendingPacketforContrRcv = NULL;
		pendingSocket.pendingBytesforContrRcv = 0;
		m_pendingSocket[socket] = pendingSocket;
	}
	do
	{
		if (!m_pendingSocket[socket].pendingBytesforContrRcv)
		{
			...
			ofp_header header;
			m_pendingSocket[socket].pendingPacketforContrRcv = socket->RecvFrom (std::numeric_limits<uint32_t>::max (), 0, m_pendingSocket[socket].fromforContrRcv);
			m_pendingSocket[socket].pendingPacketforContrRcv->CopyData ((uint8_t*)&header, sizeof (ofp_header));
			m_pendingSocket[socket].pendingBytesforContrRcv = ntohs (header.length) - m_pendingSocket[socket].pendingPacketforContrRcv->GetSize();
		}

		// Receive the remaining OpenFlow message
		if (m_pendingSocket[socket].pendingBytesforContrRcv)
		{
			if (socket->GetRxAvailable () < m_pendingSocket[socket].pendingBytesforContrRcv)
			{
				// We need to wait for more bytes
				return;
			}
			Ptr<Packet> recTemp = socket->Recv (m_pendingSocket[socket].pendingBytesforContrRcv, 0);
			if (recTemp == NULL) return;
			m_pendingSocket[socket].pendingPacketforContrRcv->AddAtEnd (recTemp);
		}

		if (InetSocketAddress::IsMatchingType (m_pendingSocket[socket].fromforContrRcv))
		{
			Ipv4Address ipv4 = InetSocketAddress::ConvertFrom (m_pendingSocket[socket].fromforContrRcv).GetIpv4 ();
			uint16_t port = InetSocketAddress::ConvertFrom (m_pendingSocket[socket].fromforContrRcv).GetPort ();

			NS_LOG_DEBUG ("At time " << Simulator::Now ().GetSeconds () <<
					"s the OpenFlow switch " << GetDatapathId () <<
					" received " << m_pendingSocket[socket].pendingPacketforContrRcv->GetSize () <<
			...
			if (InetSocketAddress::ConvertFrom (m_pendingSocket[socket].fromforContrRcv) == m_ctrlAddr) {
				ctrl.conn_id = 0;
			} else {
			...
			ofpbuf *buffer = ofs::BufferFromPacket (m_pendingSocket[socket].pendingPacketforContrRcv,
					m_pendingSocket[socket].pendingPacketforContrRcv->GetSize ());
			...
			if (!error)
			{
				...
				Address des = InetSocketAddress(ipv4,port);
				if (!m_udpConnStat[des].TxHello) {
					// send hello msg on the udp auxiliary connection
					int i = 0;
					for (std::map<Address,Ptr<Socket> >::iterator it = m_auxConn.begin(); it != m_auxConn.end(); ++it) {
						if (it->second == socket) {
							ofl_msg_header hello_msg;
							hello_msg.type = OFPT_HELLO;
							struct sender mysender = {remote:NULL, conn_id:(uint8_t)(i+1),xid:0};
							dp_send_message (m_datapath, &hello_msg, &mysender);
							break;
						}
						i++;
					}
				}
				if (m_udpConnStat.find(des) == m_udpConnStat.end()) {
					std::cout<< "receive unknown message" << std::endl;
					NS_LOG_ERROR("Receive unknown Msg ");
				} else {
					if (msg->type == OFPT_HELLO) {
						m_udpConnStat[des].RecHello = true;
					} else {
						if (!m_udpConnStat[des].RecHello) {
							NS_LOG_ERROR("receive another msg before receiving a hello msg, send a error msg back to the controller.");
							ofl_msg_error err;
					...
					m_udpConnStat[des].LastRecTime = Simulator::Now();
					if (m_udpEventId[des].PeekEventImpl() == NULL) {
						m_udpEventId[des] = Simulator::Schedule (m_noRecTimeOut, &OFSwitch13Device::UdpAuxConnNoRecTimeout, this, des);;
					} else {
						Simulator::Cancel(m_udpEventId[des]);
						m_udpEventId[des] = Simulator::Schedule (m_noRecTimeOut, &OFSwitch13Device::UdpAuxConnNoRecTimeout, this, des);;
					}
				}

				if (msg->type == OFPT_PACKET_OUT) {
					if (m_pkt_in.find(((ofl_msg_packet_out*)msg)->buffer_id) == m_pkt_in.end()) {
						//std::cout << "receive unknown pkt_in msg" << std::endl;
						NS_LOG_ERROR("Receive unknown Packet-In Msg ");
					} else {
						m_flow_setup[((ofl_msg_packet_out*)msg)->buffer_id] = Simulator::Now().GetSeconds() - m_pkt_in[((ofl_msg_packet_out*)msg)->buffer_id];
					}
				}
		...
		m_pendingSocket[socket].pendingPacketforContrRcv = 0;
		m_pendingSocket[socket].pendingBytesforContrRcv = 0;
...
void OFSwitch13Device::ReceiveFromController(Ptr<Socket> socket) {
	NS_LOG_FUNCTION(this << socket);
	if (m_pendingSocket.find(socket) == m_pendingSocket.end()) {
		PendingRecv pendingSocket;
		pendingSocket.pendingPacketforContrRcv = NULL;
	...
	do {
		if (!m_pendingSocket[socket].pendingBytesforContrRcv) {
			...
			// Receive the OpenFlow header and get the OpenFlow message size
			ofp_header header;
			m_pendingSocket[socket].pendingPacketforContrRcv = socket->RecvFrom(
					sizeof(ofp_header), 0,
					m_pendingSocket[socket].fromforContrRcv);
			m_pendingSocket[socket].pendingPacketforContrRcv->CopyData(
					(uint8_t*) &header, sizeof(ofp_header));
			m_pendingSocket[socket].pendingBytesforContrRcv = ntohs(
					header.length) - sizeof(ofp_header);
		}

		// Receive the remaining OpenFlow message
		if (m_pendingSocket[socket].pendingBytesforContrRcv) {
			if (socket->GetRxAvailable()
					< m_pendingSocket[socket].pendingBytesforContrRcv) {
				// We need to wait for more bytes
				return;
			}
			m_pendingSocket[socket].pendingPacketforContrRcv->AddAtEnd(
					socket->Recv(
							m_pendingSocket[socket].pendingBytesforContrRcv,
							0));
		}
		...
		if (InetSocketAddress::IsMatchingType(
				m_pendingSocket[socket].fromforContrRcv)) {
			Ipv4Address ipv4 = InetSocketAddress::ConvertFrom(
					m_pendingSocket[socket].fromforContrRcv).GetIpv4();
			uint16_t port = InetSocketAddress::ConvertFrom(
					m_pendingSocket[socket].fromforContrRcv).GetPort();

			NS_LOG_DEBUG(
					"At time " << Simulator::Now ().GetSeconds () << "s the OpenFlow switch " << GetDatapathId () << " received " << m_pendingSocket[socket].pendingPacketforContrRcv->GetSize () << " bytes from controller " << ipv4 << " socket " << socket << " port " << port);

			ofl_msg_header *msg;
			ofl_err error;

			struct sender ctrl;
			ctrl.remote = CONTAINER_OF(list_front(&m_datapath->remotes), remote,
					node);
			if (InetSocketAddress::ConvertFrom(
					m_pendingSocket[socket].fromforContrRcv) == m_ctrlAddr) {
				ctrl.conn_id = 0;
			} else {
				for (std::map<uint8_t, Address>::iterator it =
						m_aux_id_to_addr.begin(); it != m_aux_id_to_addr.end();
						++it) {
					if (it->second == InetSocketAddress(ipv4, port)) {
						ctrl.conn_id = it->first;
						break;
			...
			// Get the OpenFlow buffer, unpack the message and send to handler
			ofpbuf *buffer =
					ofs::BufferFromPacket(
							m_pendingSocket[socket].pendingPacketforContrRcv,
							m_pendingSocket[socket].pendingPacketforContrRcv->GetSize());
			error = ofl_msg_unpack((uint8_t*) buffer->data, buffer->size, &msg,
					&ctrl.xid, m_datapath->exp);
			...
			if (!error) {
				...
				if ((m_conn_to_RXthr[ctrl.conn_id]).find(msg->type)
						!= (m_conn_to_RXthr[ctrl.conn_id]).end()) {
					(m_conn_to_RXthr[ctrl.conn_id])[msg->type] += buffer->size;
				} else {
					(m_conn_to_RXthr[ctrl.conn_id])[msg->type] = buffer->size;
				}
				...
				if (msg->type == OFPT_PACKET_OUT) {
					//std::cout << "receive a packet out" << std::endl;
					if (m_pkt_in.find(((ofl_msg_packet_out*) msg)->buffer_id)
							== m_pkt_in.end()) {
						//std::cout << "receive unknown pkt_in msg" << std::endl;
						NS_LOG_ERROR("Receive unknown Packet-In Msg ");
					} else {
						m_flow_setup[((ofl_msg_packet_out*) msg)->buffer_id] =
								Simulator::Now().GetSeconds()
										- m_pkt_in[((ofl_msg_packet_out*) msg)->buffer_id];
					}
				}
				error = handle_control_msg(m_datapath, msg, &ctrl);
				if (error) {
...
void OFSwitch13Device::SocketCtrlSucceeded(Ptr<Socket> socket) {
	NS_LOG_FUNCTION(this << socket);

	NS_LOG_DEBUG(
			"Switch " << m_datapath << ": Controller accepted main connection request!");
...
void OFSwitch13Device::SocketAuxCtrlSucceeded(Ptr<Socket> socket) {
	...
	ofl_msg_header msg;
	msg.type = OFPT_HELLO;
struct sender mysender = {remote:NULL,conn_id:1,xid:0}
;
...
dp_send_message (m_datapath, &msg, &mysender);
}

void OFSwitch13Device::SocketCtrlFailed(Ptr<Socket> socket) {
NS_LOG_FUNCTION(this << socket);
NS_LOG_ERROR("Controller did not accepted connection request!");
}

void OFSwitch13Device::NotifyMeterEntryCreated(struct meter_entry *entry) {
...
void OFSwitch13Device::NotifyPacketCloned(struct packet *pkt,
	struct packet *clone) {
...
void OFSwitch13Device::NotifyPacketDestroyed(struct packet *pkt) {
NS_LOG_FUNCTION(this << pkt->ns3_uid);
...
if (m_pktPipe.IsValid() && m_pktPipe.HasId(pkt->ns3_uid)) {
	...
	if (!valid) {
		NS_LOG_DEBUG(
				"Packet " << pkt->ns3_uid << " done at switch " << GetDatapathId ());
	}
} else {
...
void OFSwitch13Device::NotifyPacketDropped(struct packet *pkt) {
NS_LOG_FUNCTION(this << pkt->ns3_uid);
...
void OFSwitch13Device::BufferPacketSave(uint64_t packetId, time_t timeout) {
...
if (ret.second == false) {
	NS_LOG_WARN(
			"Packet " << packetId << " already in switch " << GetDatapathId () << " buffer.");
}
...
void OFSwitch13Device::BufferPacketRetrieve(uint64_t packetId) {
NS_LOG_FUNCTION(this << packetId);
...
void OFSwitch13Device::BufferPacketDelete(uint64_t packetId) {
NS_LOG_FUNCTION(this << packetId);
...
uint64_t OFSwitch13Device::GetNewPacketId() {
return ++m_globalPktId;
}

bool OFSwitch13Device::CopyTags(Ptr<const Packet> srcPkt,
...
void OFSwitch13Device::RegisterDatapath(uint64_t id,
...
void OFSwitch13Device::UnregisterDatapath(uint64_t id) {
...
Ptr<OFSwitch13Device> OFSwitch13Device::GetDevice(uint64_t id) {
DpIdDevMap_t::iterator it;
it = OFSwitch13Device::m_globalSwitchMap.find(id);
if (it != OFSwitch13Device::m_globalSwitchMap.end()) {
	return it->second;
...
OFSwitch13Device::PipelinePacket::PipelinePacket() :
	m_valid(false), m_packet(0) {
}

void OFSwitch13Device::PipelinePacket::SetPacket(uint64_t id,
	Ptr<Packet> packet) {
...
Ptr<Packet> OFSwitch13Device::PipelinePacket::GetPacket(void) const {
NS_ASSERT_MSG(IsValid(), "Invalid packet metadata.");
return m_packet;
}

void OFSwitch13Device::PipelinePacket::Invalidate(void) {
m_valid = false;
m_packet = 0;
m_ids.clear();
}

bool OFSwitch13Device::PipelinePacket::IsValid(void) const {
return m_valid;
}

void OFSwitch13Device::PipelinePacket::NewCopy(uint64_t id) {
NS_ASSERT_MSG(m_valid, "Invalid packet metadata.");
m_ids.push_back(id);
}

bool OFSwitch13Device::PipelinePacket::DelCopy(uint64_t id) {
NS_ASSERT_MSG(m_valid, "Invalid packet metadata.");
...
bool OFSwitch13Device::PipelinePacket::HasId(uint64_t id) {
NS_ASSERT_MSG(m_valid, "Invalid packet metadata.");
