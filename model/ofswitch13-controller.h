...
struct SwitchInfo {
	...
	Ipv4Address ctrl_ipv4;            //!< controller ipv4 address
	...
	Ptr<Socket> socket;               //!< socket connected to controller
...
public:
	...
	//int m_numPktIn;  // the number of packet in received
	/**
	 * Register this type.
	...
	/**
	 * \brif register connection information on this controller
	 * \param datapathID the datapath id of the switch
	 * \param auxID the  auxiliary id of the auxiliary connection
	 * \param source the ipv4 address of the connection on switch side
	 */
	void RegisterConnection(uint8_t auxID, uint64_t datapathID,
			Ipv4Address source);

	void UdpAuxConnFeaReqTimeout(SwitchInfo* swtch, Address addr);
...
protected:
	...
	virtual ofl_err
	HandleHello(ofl_msg_header *msg, SwitchInfo swtch, uint32_t xid);
	...
	std::map<Ipv4Address, std::pair<uint64_t, uint8_t> > m_connectionMap; // !< map the address to connection id(datapath_id, aux_id)
	std::map<uint64_t, bool> m_mainConnSetup; // record whether the main connection is built
...
private:
	 ...
	 */
	//\{
	/** Receive tcp packet from switch */
	void SocketRead(Ptr<Socket> socket);

	/** Receive tcp packet from switch */
	void SocketUdpRead(Ptr<Socket> socket);

	/**
	 * select a random connection among the connections between one switch and the controller
	 * \param sw The given switch connection
	 * return the randomly selected switch connection
	 */
	SwitchInfo SelectRandomSW(SwitchInfo sw);

	/**
	 * select a random tcp connection among the connections between one switch and the controller
	 * \param sw The given switch connection
	 * return the randomly selected switch connection
	 */
	SwitchInfo SelectRandomTCPSW(SwitchInfo sw);
