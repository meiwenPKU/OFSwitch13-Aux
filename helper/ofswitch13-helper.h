...
public:
   ...
   * \param swNode The node to install the device in
   * \param ports Container of NetDevices to add as switch ports
   * \param v_auxConn auxiliary connection type
   * \returns A container holding the OFSwitch13Device devices.
   */
  OFSwitch13DeviceContainer InstallSwitchWithAux (
    Ptr<Node> swNode, NetDeviceContainer ports, std::vector<SocketType>& v_auxConn);
