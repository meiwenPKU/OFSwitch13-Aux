...
OFSwitch13Helper::InstallSwitch (Ptr<Node> swNode, NetDeviceContainer ports)
{
  ...
  //------debug------
  std::cout << "main connection controller ip address = " << ctrlAddr << std::endl;
  std::cout << "main connection switch ip address = " << switchAddr << std::endl;
  //-----
 ...
 */
OFSwitch13DeviceContainer
OFSwitch13Helper::InstallSwitchWithAux (Ptr<Node> swNode, NetDeviceContainer ports, std::vector<SocketType>& v_auxConn)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT_MSG (m_ctrlNode, "Install the controller before switch.");
  NS_LOG_DEBUG ("Install OpenFlow switch device on node " << swNode->GetId ());
  ...
  if (m_ctrlApp)
    {
      m_ctrlApp->RegisterSwitchMetadata (swInfo);
      m_ctrlApp->RegisterConnection(0,openFlowDev->GetDatapathId(),swInfo.ipv4);
    }

  //----install the auxiliary connections
  uint8_t aux_id = 1;
 // Address aux_switchAddr;
  for (std::vector<SocketType>::iterator it = v_auxConn.begin(); it != v_auxConn.end(); ++it)
  {
	    ...
	    case OFSwitch13Helper::DEDICATEDCSMA:
	      {
	        ...
	        aux_ctrlAddr = InetSocketAddress (aux_swIface.GetAddress (1), m_ctrlPort);
	       // aux_switchAddr = InetSocketAddress (aux_swIface.GetAddress (0), m_swPort);
	        break;
	      }
	    case OFSwitch13Helper::DEDICATEDP2P:
	      {
	        // Create a dedicated p2p link between switch and controller
	        NetDeviceContainer aux_swDev = m_p2pHelper.Install (swNode, m_ctrlNode);
	        aux_swIface = m_ipv4helper.Assign (aux_swDev);
	        m_ipv4helper.NewNetwork ();
	        m_ctrlDevs.Add (aux_swDev.Get (1));
	        aux_ctrlAddr = InetSocketAddress (aux_swIface.GetAddress (1), m_ctrlPort);
	  ...
	  if (m_ctrlApp)
	    {
	      ...
	      m_ctrlApp->RegisterConnection(aux_id,openFlowDev->GetDatapathId(),aux_swInfo.ipv4);
	    }
	  openFlowDev->SetAuxConn(aux_ctrlAddr,aux_id); // ?? cast InetSocketAddress to Address
	  aux_id++;
  }
