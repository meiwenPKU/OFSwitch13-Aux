...
namespace ns3 {
	...
	m_csmaChannel(0), m_ctrlNode(0), m_ctrlApp(0), m_ctrlPort(6653), m_swPort(
			9999) {
	...
	TypeId OFSwitch13Helper::GetTypeId(void) {
		static TypeId tid =
		TypeId("ns3::OFSwitch13Helper").SetParent<Object>().SetGroupName(
				"OFSwitch13").AddConstructor<OFSwitch13Helper>().AddAttribute(
				"ChannelType",
				...
				MakeEnumChecker(OFSwitch13Helper::SINGLECSMA, "SingleCsma",
						...
						OFSwitch13Helper::DEDICATEDP2P, "DedicatedP2p")).AddAttribute(
				"ChannelDataRate",
	...
	void OFSwitch13Helper::DoDispose() {
		NS_LOG_FUNCTION(this);
		m_ctrlNode = 0;
		m_ctrlApp = 0;
		m_csmaChannel = 0;
	...
	void OFSwitch13Helper::SetDeviceAttribute(std::string n1,
			const AttributeValue &v1) {
		NS_LOG_FUNCTION(this);
		m_devFactory.Set(n1, v1);
	}

	void OFSwitch13Helper::SetChannelType(ChannelType type) {
	...
	void OFSwitch13Helper::SetChannelDataRate(DataRate datarate) {
		...
		switch (m_channelType) {
			...
			case OFSwitch13Helper::DEDICATEDCSMA: {
				...
				m_csmaHelper.SetQueue("ns3::DropTailQueue", "MaxPackets",
						UintegerValue(65536));
				break;
			}
			case OFSwitch13Helper::DEDICATEDP2P: {
				...
				m_p2pHelper.SetQueue("ns3::DropTailQueue", "MaxPackets",
						UintegerValue(65536));
				break;
			}
			default: {
	...
	void OFSwitch13Helper::EnableDatapathLogs(std::string level) {
	...
	void OFSwitch13Helper::SetAddressBase(Ipv4Address network, Ipv4Mask mask,
			Ipv4Address base) {
		switch (m_channelType) {
			case OFSwitch13Helper::SINGLECSMA: {
	...
	OFSwitch13DeviceContainer OFSwitch13Helper::InstallSwitchesWithoutPorts(
	...
	Ptr<OFSwitch13Controller> OFSwitch13Helper::InstallDefaultController(
			Ptr<Node> cNode) {
		NS_LOG_FUNCTION(this);
	...
	OFSwitch13DeviceContainer OFSwitch13Helper::InstallSwitch(Ptr<Node> swNode,
		...
		for (NetDeviceContainer::Iterator i = ports.Begin(); i != ports.End();
				++i) {
		...
		switch (m_channelType) {
			case OFSwitch13Helper::SINGLECSMA: {
				...
				ctrlAddr = m_ctrlAddr;
				switchAddr = InetSocketAddress(swIface.GetAddress(0), m_swPort);
				break;
			}
			case OFSwitch13Helper::DEDICATEDCSMA: {
		...
		swInfo.ctrl_ipv4 = swIface.GetAddress(1);
		swInfo.swDev = openFlowDev;
		swInfo.node = swNode;
		swInfo.port = m_swPort;
		if (m_ctrlApp) {
			m_ctrlApp->RegisterSwitchMetadata(swInfo);
			m_ctrlApp->RegisterConnection(0, openFlowDev->GetDatapathId(),
					swInfo.ipv4);
		}
		openFlowDev->SetAttribute("ControllerAddr", AddressValue(ctrlAddr));
		openFlowDev->SetAttribute("SwitchAddr", AddressValue(switchAddr));
	...
	/**
	 * enable installing multiple connections between the switch and the controller
	 */
	OFSwitch13DeviceContainer OFSwitch13Helper::InstallSwitchWithAux(
			Ptr<Node> swNode, NetDeviceContainer ports,
			std::vector<SocketType>& v_auxConn) {
		NS_LOG_FUNCTION(this);
		NS_ASSERT_MSG(m_ctrlNode, "Install the controller before switch.");
		NS_LOG_DEBUG("Install OpenFlow switch device on node " << swNode->GetId ());
		...
		//----install the auxiliary connections
		uint8_t aux_id = 1;
		// Address aux_switchAddr;
		for (std::vector<SocketType>::iterator it = v_auxConn.begin();
				it != v_auxConn.end(); ++it) {
			Address aux_ctrlAddr;
			Address aux_switchAddr;
			Ipv4InterfaceContainer aux_swIface;
			switch (m_channelType) {
				case OFSwitch13Helper::SINGLECSMA: {
					// Connecting the switch to common csma network
					NetDeviceContainer aux_swDev = m_csmaHelper.Install(swNode,
							m_csmaChannel);
					aux_swIface = m_ipv4helper.Assign(aux_swDev);
					aux_ctrlAddr = m_ctrlAddr;
					aux_switchAddr = InetSocketAddress(aux_swIface.GetAddress(0),
							m_swPort);
					break;
				}
				case OFSwitch13Helper::DEDICATEDCSMA: {
					...
					NetDeviceContainer aux_swDev = m_csmaHelper.Install(pair);
					aux_swIface = m_ipv4helper.Assign(aux_swDev);
					m_ipv4helper.NewNetwork();
					m_ctrlDevs.Add(aux_swDev.Get(1));
					aux_ctrlAddr = InetSocketAddress(aux_swIface.GetAddress(1),
							m_ctrlPort);
					aux_switchAddr = InetSocketAddress(aux_swIface.GetAddress(0),
							m_swPort);
					break;
				...
				case OFSwitch13Helper::DEDICATEDP2P: {
					// Create a dedicated p2p link between switch and controller
					NetDeviceContainer aux_swDev = m_p2pHelper.Install(swNode,
							m_ctrlNode);
					aux_swIface = m_ipv4helper.Assign(aux_swDev);
					m_ipv4helper.NewNetwork();
					m_ctrlDevs.Add(aux_swDev.Get(1));
			...
			SwitchInfo aux_swInfo;
			aux_swInfo.ipv4 = aux_swIface.GetAddress(0);
			aux_swInfo.ctrl_ipv4 = aux_swIface.GetAddress(1);
			aux_swInfo.swDev = openFlowDev;
			aux_swInfo.node = swNode;
			aux_swInfo.port = m_swPort;

			if ((*it) == UDP) {
				Ptr<Socket> send_socket = Socket::CreateSocket(m_ctrlNode,
						UdpSocketFactory::GetTypeId());
				aux_swInfo.socket = send_socket;
			}

			//debug
			if ((*it) == UDP) {
				Ipv4Address de_ipv4 =
				InetSocketAddress::ConvertFrom(aux_ctrlAddr).GetIpv4();
				uint16_t de_port =
				InetSocketAddress::ConvertFrom(aux_ctrlAddr).GetPort();
				NS_LOG_DEBUG(de_ipv4 << " " << de_port);
			}
			if (m_ctrlApp) {
				m_ctrlApp->RegisterSwitchMetadata(aux_swInfo);
				m_ctrlApp->RegisterConnection(aux_id, openFlowDev->GetDatapathId(),
						aux_swInfo.ipv4);
			}

			openFlowDev->SetAuxConn(aux_ctrlAddr, aux_switchAddr, aux_id); // ?? cast InetSocketAddress to Address

			aux_id++;
		}

		openFlowDev->StartControllerConnection();
		openFlowDev->StartControllerAuxConnection(v_auxConn);
		return OFSwitch13DeviceContainer(openFlowDev);
	}

	Ptr<OFSwitch13Controller> OFSwitch13Helper::InstallControllerApp(
		...
		switch (m_channelType) {
			case OFSwitch13Helper::SINGLECSMA: {
				...
				//m_swAddr = InetSocketAddress (ctrlIface.GetAddress (1), m_swPort);
				break;
			}
			case OFSwitch13Helper::DEDICATEDCSMA:
			case OFSwitch13Helper::DEDICATEDP2P: {
	...
	Ptr<NetDevice> OFSwitch13Helper::InstallExternalController(Ptr<Node> cNode) {
	...
	void OFSwitch13Helper::EnableOpenFlowPcap(std::string prefix) {
		NS_LOG_FUNCTION(this);
		switch (m_channelType) {
			case OFSwitch13Helper::SINGLECSMA:
			case OFSwitch13Helper::DEDICATEDCSMA: {
	...
	void OFSwitch13Helper::EnableOpenFlowAscii(std::string prefix) {
		NS_LOG_FUNCTION(this);
